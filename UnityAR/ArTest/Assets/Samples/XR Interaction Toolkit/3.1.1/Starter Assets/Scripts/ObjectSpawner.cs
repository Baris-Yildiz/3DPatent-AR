...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets
{
    ...
    public class ObjectSpawner : MonoBehaviour
    {
        ...
        private List<Boolean> isOnSceneList = new List<bool>();
        ...
        public bool TrySpawnObject(Vector3 spawnPoint, Vector3 spawnNormal)
        {
            ...
            if (isOnSceneList[objectIndex])
            {
                return false;
            }

            var newObject = Instantiate(m_ObjectPrefabs[objectIndex]);
            isOnSceneList[objectIndex] = true;
            if (m_SpawnAsChildren)
            {
                newObject.transform.parent = transform;
            }

            BoxCollider collider = m_ObjectPrefabs[objectIndex].GetComponentInChildren<BoxCollider>();
            
            if (collider == null)
            {
                collider = m_ObjectPrefabs[objectIndex].GetComponent<BoxCollider>();
            }
            ...
            if (collider != null)
            {
                ...
                pos = new Vector3(spawnPoint.x,
                    spawnPoint.y -collider.size.y * scale.y / 4, spawnPoint.z);
            }
            else
            {
                pos = spawnPoint;
        ...
        public bool objectIsOnScene(int idx)
        {
            return isOnSceneList[idx];
        }
    }
